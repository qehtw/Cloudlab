...
-- Створюємо нову базу
CREATE DATABASE dblab CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Використовуємо цю базу
USE dblab;
...
CREATE TABLE repairs (
    ...
    status ENUM('in_progress', 'completed', 'pending') NOT NULL,
    start_date DATE NOT NULL,
    end_date DATE NULL,
    repairs_name VARCHAR(100),
    FOREIGN KEY (user_equipment_id) REFERENCES user_equipment(user_equipment_id) ON DELETE CASCADE,
    FOREIGN KEY (repair_type_id) REFERENCES repair_types(repair_type_id) ON DELETE CASCADE
);
...
CREATE TABLE technician_repairs (
...
-- кожен технік працює над кожним ремонтом
INSERT INTO technician_repairs (technician_id, repair_id)
SELECT technician_id, repair_id FROM technicians, repairs;
